...
func (bc *BranchCreate) SetFacultyID(id int) *BranchCreate {
	bc.mutation.SetFacultyID(id)
	return bc
}

// SetNillableFacultyID sets the faculty edge to Faculty by id if the given value is not nil.
func (bc *BranchCreate) SetNillableFacultyID(id *int) *BranchCreate {
	if id != nil {
		bc = bc.SetFacultyID(*id)
	}
	return bc
}

// SetFaculty sets the faculty edge to Faculty.
func (bc *BranchCreate) SetFaculty(f *Faculty) *BranchCreate {
	return bc.SetFacultyID(f.ID)
}
...
func (bc *BranchCreate) createSpec() (*Branch, *sqlgraph.CreateSpec) {
	...
	if nodes := bc.mutation.FacultyIDs(); len(nodes) > 0 {
