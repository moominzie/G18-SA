...
func (e RoomEdges) BuildingOrErr() (*Building, error) {
	if e.loadedTypes[0] {
		if e.Building == nil {
...
func (r *Room) assignValues(values ...interface{}) error {
	...
	if len(values) == len(room.ForeignKeys) {
		if value, ok := values[0].(*sql.NullInt64); !ok {
			...
			r.building_id = new(int)
			*r.building_id = int(value.Int64)
		}
	}
	return nil
}

// QueryBuilding queries the building edge of the Room.
func (r *Room) QueryBuilding() *BuildingQuery {
	return (&RoomClient{config: r.config}).QueryBuilding(r)
}
