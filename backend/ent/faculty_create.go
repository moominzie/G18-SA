...
func (fc *FacultyCreate) AddBranchIDs(ids ...int) *FacultyCreate {
	fc.mutation.AddBranchIDs(ids...)
	return fc
}

// AddBranchs adds the branchs edges to Branch.
func (fc *FacultyCreate) AddBranchs(b ...*Branch) *FacultyCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return fc.AddBranchIDs(ids...)
}
...
func (fc *FacultyCreate) createSpec() (*Faculty, *sqlgraph.CreateSpec) {
	...
	if nodes := fc.mutation.BranchsIDs(); len(nodes) > 0 {
