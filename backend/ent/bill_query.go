...
type BillQuery struct {
	...
	withBillingstatus *BillingstatusQuery
	withFKs           bool
...
func (bq *BillQuery) QueryRepairinvoice() *RepairInvoiceQuery {
	query := &RepairInvoiceQuery{config: bq.config}
...
func (bq *BillQuery) QueryEmployee() *EmployeeQuery {
...
// QueryBillingstatus chains the current query on the Billingstatus edge.
func (bq *BillQuery) QueryBillingstatus() *BillingstatusQuery {
	query := &BillingstatusQuery{config: bq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BillQuery) WithRepairinvoice(opts ...func(*RepairInvoiceQuery)) *BillQuery {
	query := &RepairInvoiceQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withRepairinvoice = query
	return bq
}
...
func (bq *BillQuery) WithEmployee(opts ...func(*EmployeeQuery)) *BillQuery {
	query := &EmployeeQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithBillingstatus tells the query-builder to eager-loads the nodes that are connected to
// the "Billingstatus" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithBillingstatus(opts ...func(*BillingstatusQuery)) *BillQuery {
	query := &BillingstatusQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withBillingstatus = query
	return bq
}
...
func (bq *BillQuery) sqlAll(ctx context.Context) ([]*Bill, error) {
	var (
		...
		_spec       = bq.querySpec()
		loadedTypes = [3]bool{
			bq.withRepairinvoice != nil,
			bq.withEmployee != nil,
			bq.withBillingstatus != nil,
		}
	)
	if bq.withRepairinvoice != nil || bq.withEmployee != nil || bq.withBillingstatus != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := bq.withRepairinvoice; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].bill_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "bill_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := bq.withBillingstatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].billingstatus_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(billingstatus.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "billingstatus_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Billingstatus = n
			}
		}
	}
